...
func _input(event):
	...
	if event.is_action_released("move_piece") and moving:
		...
		parent.get_parent().get_node("Pieces").add_child(self)
...
func set_type(type):
	self.type = type
	set_normal_texture(global.get_piece_texture(type, color))
	if type == 7:
		set_draw_behind_parent(true)
	if type < 7:
		highlight_node.set_texture(global.get_piece_highlight_texture(type))
		if color == 1:
			bg_node.set_texture(global.get_piece_bg_texture(type))
			bg_node.show()
		else:
			bg_node.hide()
...
func _on_Piece_button_down():
	if movable and !global.is_fading() and !moving:
		if type != 7 or global.get_editor():
			moving = true
			var parent = get_parent()
			parent.remove_child(self)
			parent.get_parent().get_node("PickedUp").add_child(self)
