...
func clear():
	for x in range(board_size.x):
		for y in range(board_size.y):
			...
			grid[x][y].status[0] = 0
			grid[x][y].status[1] = 0
			grid[x][y].protectors[0] = []
			grid[x][y].protectors[1] = []
...
func add_piece(piece):
	...
	grid[x][y].color = piece.get_color()
...
func check_solved():
	...
	for x in range(board_size.x):
		for y in range(board_size.y):
			if grid[x][y].type != 0 and grid[x][y].type != 7 and grid[x][y].status[grid[x][y].color] != 1:
				return false
...
func mark(piece, pos):
	...
	# Mark the position.
	grid[pos.x][pos.y].status[piece.get_color()] += 1
	grid[pos.x][pos.y].protectors[piece.get_color()].append(piece)
...
func get_piece_status(piece):
	...
	return grid[x][y].status[piece.get_color()]
